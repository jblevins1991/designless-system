...
const Code: React.FC<CodeProps> = ({
  ...
  return (
    <div className={classNames('code-root', className)} id={`${id}-code-root`}>
      <div className="code-header" id={`${id}-code-header`}>
        <span>{language}</span>

        <button className="code-copy-button" onClick={handleCopyClick}>
          {copyIcon}
        </button>
      </div>
      <span className="code-content" id={`${id}-code-content`} ref={codeRef}>
        {children}
      </span>
    </div>
  );
